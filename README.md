...
Paired evaluation framework for structured field extraction (`baseline.csv` vs `new.csv`).
...
- `src/prompt_stat_eval/`
  ...
  - `normalize.py`: missing/date/number/code/text normalization and matching
...
- parse-error threshold fixed at `5%` for `date`, `number`, `rate`
- expected value mismatch across baseline/new keys causes hard fail
