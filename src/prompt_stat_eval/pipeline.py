...
def run_evaluation(
    ...
    untracked_row_count = len(base_df.merge(new_df, on=["record_id", "field_name"], how="inner")) - len(paired)
    ...
    segment_metrics = build_segment_metrics(paired)
    rollout = build_rollout_decision(field_metrics, overall_metrics["overall_lift_gate_pass"])

    mismatch_rows = None
    if mismatch_counts["record_type_mismatch_count"] or mismatch_counts["document_group_mismatch_count"]:
        mismatch_rows = paired[
            (paired["record_type_mismatch"] == 1) | (paired["document_group_mismatch"] == 1)
        ].copy()
