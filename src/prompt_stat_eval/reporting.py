...
def write_review_checklist(
    ...
    lines.append(f"- record_type mismatch count: {mm['record_type_mismatch_count']}")
    lines.append(f"- document_group mismatch count: {mm['document_group_mismatch_count']}")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Segment Summary (record_type/document_group)")
    lines.append("| record_type | document_group | N | Acc_old | Acc_new | Lift | b | c | McNemar p |")
    lines.append("|---|---|---:|---:|---:|---:|---:|---:|---:|")
    for _, row in segment_metrics.iterrows():
        lines.append(
            f"| {row['record_type']} | {row['document_group']} | {int(row['N'])} | "
...
def write_artifacts(
    ...
    segment_out = output_dir / "segment_metrics.csv"
    rollout_out = output_dir / "rollout_decision.csv"
    checklist_out = output_dir / "review_checklist.md"
    ...
    write_review_checklist(checklist_out, overall_metrics, field_metrics, rollout, segment_metrics)

    outputs = {
        "paired_eval_csv": str(paired_out),
        "overall_metrics_json": str(overall_out),
        "field_metrics_csv": str(field_out),
        "segment_metrics_csv": str(segment_out),
        "rollout_decision_csv": str(rollout_out),
        "review_checklist_md": str(checklist_out),
    }
