...
def validate_parse_error_thresholds(paired: pd.DataFrame) -> Dict[str, dict]:
    stats: Dict[str, dict] = {}
    for ftype in ["date", "number", "rate"]:
        base_mask = (paired["field_type"] == ftype) & (paired["expected_present"] == 1)
...
def build_field_metrics(paired: pd.DataFrame, cfg: EvalConfig) -> pd.DataFrame:
    ...
    for field_name in ALL_TRACKED_FIELDS:
        ...
        if tier == "CRITICAL":
            crit_base = sub[sub["expected_present"] == 1]
...
def build_segment_metrics(paired: pd.DataFrame) -> pd.DataFrame:
    rows: List[dict] = []

    for (record_type, document_group), sub in paired.groupby(["record_type", "document_group"], dropna=False):
        b = int(sub["improvement"].sum())
        c = int(sub["regression"].sum())

        crit_ubs: List[float] = []
        for field in CRITICAL_FIELDS:
            crit_sub = sub[(sub["field_name"] == field) & (sub["expected_present"] == 1)]
    ...
    return pd.DataFrame(rows).sort_values(["record_type", "document_group"]).reset_index(drop=True)
